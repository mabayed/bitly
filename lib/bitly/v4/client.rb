  ...
  module V4
    ...
    class Client
      ...
      def shorten(long_url, opts={})
        ...
        response = post('/shorten', :body => body)
      ...
      def lookup(input)
        ...
        results = response['data']['lookup'].inject([]) do |results, url|
          ...
          if url['error'].nil?
            ...
            results[input.index(url['long_url'])] = Bitly::V4::Url.new(self, url)
            # remove the key from the original array, in case the same hash/url was entered twice
            input[input.index(url['long_url'])] = nil
          else
            results[input.index(url['long_url'])] = Bitly::V4::MissingUrl.new(url)
            input[input.index(url['long_url'])] = nil
          end
          results
        end
      ...
      def get_single_method(method, input)
        ...
        query = "/#{method}?" + query
        response = get(query)
        return Bitly::V4::Url.new(self,response['data'])
      end
      ...
      def get_method(method, input, opts={})
        ...
        results = response['data'][method.to_s].inject([]) do |results, url|
          ...
          if url['error'].nil?
            # builds the results array in the same order as the input
            results[result_index] = Bitly::V4::Url.new(self, url)
            # remove the key from the original array, in case the same hash/url was entered twice
            input[result_index] = nil
          else
            results[result_index] = Bitly::V4::MissingUrl.new(url)
            input[result_index] = nil
          end
          results
        end
